# redis-index-manager

`redis-index-manager` 是一个基于 Hash 分桶策略的高性能 Redis 二级索引管理器。它专为解决海量数据下的索引热点问题而设计，支持高效的范围查询（Range Scan）和原子性的数据操作。

**注意，不适用与百万及一下数据量。该管理器的设计会导致数据问题。**

## 🌟 核心特性

- **Hash 分桶 (Sharding)**: 将索引数据均匀打散到多个 ZSET 中（默认 256 个桶），避免单 Key 热点问题。
- **Scatter-Gather 查询**: 范围查询时自动并发扫描所有相关桶，并在内存中进行归并排序。
- **内存保护**: 采用分批合并 (Incremental Merge) 策略，有效防止大范围 Scan 导致的内存溢出 (OOM)。
- **原子操作**: 使用 Lua 脚本保证数据 (KV) 与索引 (ZSET) 的强一致性。

---

## 📦 安装

```bash
npm install redis-index-manager
```

---

## 🚀 快速开始

### 1. 初始化

支持传入 ioredis 实例（兼容旧代码）或 Redis 配置对象（推荐，支持延迟连接）。

```javascript
import Redis from "ioredis";
import { RedisIndexManager } from "redis-index-manager";

// 方式 A: 传入 ioredis 实例
const redis = new Redis();
const manager = new RedisIndexManager({
  redis: redis,
  indexPrefix: "idx:", 
  hashChars: 2
});

// 方式 B: 传入配置对象 (Lazy Connect)
// 连接会在第一次调用 add/scan 等方法时自动建立
const managerLazy = new RedisIndexManager({
  redis: {
    host: "127.0.0.1",
    port: 6379,
    // 其他 ioredis 配置...
  },
  indexPrefix: "idx:",
  hashChars: 2
});
```

### 2. 添加数据

```javascript
// 会同时在 Redis 写入 "user:1001" (String) 和 更新索引 (ZSET)
await manager.add("user:1001", JSON.stringify({ name: "Alice", age: 30 }));
```

### 3. 范围查询

```javascript
// 查询 user:1000 到 user:1005 之间的数据
// 返回格式: [key1, val1, key2, val2, ...]
const results = await manager.scan("user:1000", "user:1005", 10);

for (let i = 0; i < results.length; i += 2) {
  const key = results[i];
  const val = results[i + 1];
  console.log(key, val);
}
```

### 4. 删除数据

```javascript
// 原子删除数据和索引
await manager.del("user:1001");
```

---

## 📚 API 详解

### `constructor(options)`

| 参数 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `redis` | `Redis \| Object` | - | **必填**。ioredis 实例，或 ioredis 构造函数配置对象（支持 Lazy Connect）。 |
| `indexPrefix` | `string` | `"idx:"` | 索引 ZSET 的 Key 前缀。 |
| `hashChars` | `number` | `2` | Hash 分桶位数 (Hex)。<br>`1`: 16 桶 (适合千万级)<br>`2`: 256 桶 (适合亿级，推荐) |
| `scanBatchSize` | `number` | `50` | Scan 时并发 Pipeline 的批次大小。 |
| `mgetBatchSize` | `number` | `200` | 回查 Value 时 MGET 的批次大小。 |

### `async add(key, value)`

原子性地添加数据和索引。

- **Key**: 数据的唯一标识。
- **Value**: 数据内容（字符串）。
- **注意**: 如果环境不支持 Lua 脚本，会自动降级为 Pipeline 模式，此时**不保证原子性**（控制台会输出警告）。

### `async del(key)`

原子性地删除数据和索引。

### `async scan(startKey, endKey, limit)`

执行分布式范围查询 (Scatter-Gather Scan)。

| 参数 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `startKey` | `string` | 是 | 扫描起始 Key (包含)。 |
| `endKey` | `string` | 否 | 扫描结束 Key (包含)。<br>如果未提供，尝试根据 `startKey` 的分隔符 (`_`, `:`, `-`, `/`, `#`) 自动推导前缀范围。<br>推导失败将抛出错误。 |
| `limit` | `number` | 否 | **全局**返回数量限制 (默认 100)。<br>范围: 1 - 1000。 |

- **返回值**: `Promise<Array<string>>`
  - 返回一个扁平数组，Key 和 Value 交替排列。
  - 示例: `["user:1", "val1", "user:2", "val2"]`
  - 如果未找到数据，返回空数组 `[]`。

### `async count(startKey, endKey)`

高效统计指定范围内的 Key 总数。

| 参数 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `startKey` | `string` | 是 | 统计起始 Key (包含)。 |
| `endKey` | `string` | 否 | 统计结束 Key (包含)。自动推导逻辑同 `scan`。 |

- **返回值**: `Promise<number>`
- **特性**: 纯服务端计算 (`ZLEXCOUNT`)，无需拉取数据，速度极快且不消耗 Node.js 内存。

---

## ⚠️ 限制与风险 (Limitations & Risks)

### 1. 读放大 (Read Amplification)
由于采用了 Hash 分桶策略，数据被随机打散到 256 个桶中。
- **影响**: 即使你只需要查询 `limit=5` 条数据，`scan` 方法也必须**扫描所有 256 个桶**，然后在内存中排序截取 Top 5。
- **建议**: 避免在高频（QPS > 1000）场景下调用 `scan`，或将 `hashChars` 设置为 1 以减少桶数量（但这会降低写性能）。

### 2. 原子性降级风险
- **场景**: 在某些 Redis 集群模式、Proxy 环境或 Mock 测试中，`EVAL` (Lua 脚本) 可能被禁用。
- **后果**: 代码会自动降级为 `Pipeline` 执行 `SET` + `ZADD`。
- **风险**: 如果在 `SET` 成功后发生网络故障，`ZADD` 可能失败，导致“数据存在但无法被索引查到”的数据不一致问题。

### 3. 全表扫描保护
- **机制**: 为了防止误操作导致的全表扫描（Full Scan），如果未提供 `endKey` 且无法从 `startKey` 推导范围，`scan` 方法会直接抛出 `Error`。
- **规避**: 始终显式提供 `endKey`，或确保 Key 命名符合规范（包含分隔符）。

### 4. Limit 限制
- **限制**: `limit` 最大允许设置为 **1000**。
- **原因**: 为了防止内存溢出。由于所有分桶的结果都需要汇总到 Node.js 内存中排序，过大的 Limit 配合高并发请求极易导致 Node.js 进程 OOM (Out of Memory)。

### 5. 排序成本
- **说明**: 排序是在 Node.js 内存中进行的（字典序），而不是在 Redis 端完成的。这会消耗 Node.js 的 CPU 资源。

---

## 💡 最佳实践

1. **Key 命名规范**: 使用分隔符（如 `user:1001`），以便 `scan` 自动推导范围。
2. **显式 endKey**: 在生产环境中，尽量显式传入 `endKey`，明确扫描范围。
3. **监控**: 监控 `scan` 方法的执行时间。如果耗时超过 500ms，说明桶内数据过多或并发度过高。
4. **Value 大小**: 尽量控制 Value 的大小。如果 Value 很大（如 > 10KB），`scan` 返回大量数据时会显著增加网络带宽消耗和内存压力。
